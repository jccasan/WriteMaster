    ...
    <div className="min-h-screen bg-background text-foreground font-sans">
      <header className="border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50">
        <div className="container max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={handleReset} role="button" tabIndex={0}>
      ...
      <main className="container max-w-5xl mx-auto px-4 py-8 md:py-12">
        ...
        {view === "pipeline" && projectId && (
          <StoryPipeline projectId={projectId} onComplete={handlePipelineComplete} />
        )}
        {view === "result" && projectId && (
          <StoryResult projectId={projectId} onReset={handleReset} />
        )}
      </main>
    </div>
  );
